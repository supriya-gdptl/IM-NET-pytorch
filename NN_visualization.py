...
dummy_zvec = torch.randn(1,1,256, requires_grad=True)
# dummy_zvec.to(device)
dummy_point_coord = torch.randn(1,1,3, requires_grad=True)
# dummy_point_coord.to(device)

# z_vector, _ = im_network(dummy_voxel, None, None, is_training=False)
torch.onnx.export(im_network, (dummy_voxel, dummy_zvec, dummy_point_coord), "checkpoint/imnet_architecture.onnx")
